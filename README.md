...
1uのキーに収まらない長い単語ラベル（command/control/option/shift/return/tab/delete等）は、実際のMacキーキャップの慣習に沿った記号（⌘⌃⌥⇧⇪↩⇥⌫⎋等）で表示する。KeyRaycastが自身のオーバーレイで同じキーコードに使っている記号と揃えてある。全レイアウト共通で `keycode` を軸に統一しているので、6つのJSON全部で同じキーは同じ記号になる。
