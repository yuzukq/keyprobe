...
interface Strings {
  ...
  usePreferenceSubtitle: (mode: string) => string;
...
const en: Strings = {
  ...
  usePreferenceSubtitle: (mode) =>
    `Clear this command's selection and follow the Keyboard Layout preference (currently: ${mode})`,
  autoDetectSubtitle:
  ...
  preferenceDisplayName: "Preference setting",
};

const ja: Strings = {
  builtInSection: "標準",
  customSection: "カスタムキーボード",
  usePreferenceTitle: "Preferences の設定に戻す",
  usePreferenceSubtitle: (mode) =>
    `このコマンドでの選択を解除し、Raycastの環境設定（現在: ${mode}）に従う`,
  ...
  preferenceDisplayName: "Preferences の設定",
};
